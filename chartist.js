...
(function (window, document, Chartist) {
  ...
  function dispatchEventToAndRemoveChildren (event, element) {
    element.dispatchEvent(event);
    if (element.children === undefined || element === undefined) {
      return;
    }
    ...
    for (var i = 0; i < element.children.length; i++) {
      dispatchEventToAndRemoveChildren(event, element.children[i]);
    }
  }
  ...
  Chartist.createSvg = function (container, width, height, className) {
    ...
    Array.prototype.slice.call(container.querySelectorAll('svg')).filter(function filterChartistSvgObjects(svg) {
      ...
      var event = new Event('removeFromAgenda');
      dispatchEventToAndRemoveChildren(event, svg);
    });
